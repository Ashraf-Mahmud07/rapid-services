...
export default function MunicipalityHighlight() {
  ...
  return (
    <section className="mt-10 rounded-xl bg-primary p-6 sm:p-8 lg:mt-14 lg:p-10">
      <div className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-center lg:gap-12">
        ...
        <div>
          <span className="inline-flex items-center gap-1.5 rounded-md border border-primary/40 bg-primary/10 px-2.5 py-1 text-[10px] font-semibold tracking-[1px] text-white uppercase">
            <ShieldCheckIcon className="size-3" strokeWidth={2} />
            {content.eyebrow}
          </span>
          <h2 className="mt-4 text-[26px] leading-[1.2] font-bold text-white lg:text-[30px]">
            {content.title}
            <span className="block text-white">{content.titleAccent}</span>
          </h2>
          <p className="mt-4 text-[13px] leading-[1.75] text-white">{content.description}</p>
          <div className="mt-6 flex items-start gap-3 rounded-lg bg-white/6 p-4">
            <span className="flex size-6 flex-none items-center justify-center rounded-full bg-white">
              <CheckIcon className="size-3.5 text-primary" strokeWidth={2.6} />
            </span>
            <p className="text-[13px] leading-[1.6] text-[#FFFFFF]">{content.highlight}</p>
          </div>
        </div>
      </div>
    </section>
